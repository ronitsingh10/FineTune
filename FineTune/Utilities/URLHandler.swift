...
final class URLHandler {
    ...
    func handleURL(_ url: URL) {
        ...
        case "set-listening-mode":
            handleSetListeningMode(queryItems: queryItems)
        case "connect-device":
    ...
    private func handleSetListeningMode(queryItems: [URLQueryItem]) {
        guard let modeStr = normalizedQueryValue(queryItems.first(where: { $0.name == "mode" })?.value) else {
            logger.error("set-listening-mode: missing mode parameter (use: anc, transparency, adaptive, off)")
            return
        }

        let mode: ListeningMode
        switch modeStr.lowercased() {
        case "anc", "noisecancellation", "noise-cancellation":
        ...
        case "off", "normal":
            mode = .off
        default:
            logger.error("set-listening-mode: unknown mode '\(modeStr)' (use: anc, transparency, adaptive, off)")
            return
        }

        let deviceName = normalizedQueryValue(queryItems.first(where: { $0.name == "device" })?.value)
        let btMonitor = audioEngine.bluetoothDeviceMonitor

        if let deviceName {
            if let uid = btMonitor.connectedAirPodsState.keys.first(where: { uid in
                audioEngine.outputDevices.first(where: { $0.uid == uid })?.name.localizedCaseInsensitiveContains(deviceName) == true
            }) {
                btMonitor.setListeningMode(mode, forDeviceUID: uid)
                logger.info("Set listening mode to \(mode.displayName) for \(deviceName)")
            } else {
                logger.warning("set-listening-mode: no connected AirPods found matching '\(deviceName)'")
            }
        } else {
            if let uid = btMonitor.connectedAirPodsState.keys.sorted().first {
                btMonitor.setListeningMode(mode, forDeviceUID: uid)
                logger.info("Set listening mode to \(mode.displayName) for first connected AirPods")
            } else {
                logger.warning("set-listening-mode: no connected AirPods found")
            }
        }
    }
