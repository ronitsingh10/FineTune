...
struct LoudnessEqualizerTests {
    ...
    func dbLinearRoundTrip() {
        ...
        #expect(veryNegative < -170.0, "Expected linearToDb(0) to clamp to noise floor, got \(veryNegative) dB")
    }

    @Test("dbToLinear and linearToDb handle extreme values like NaN and Infinity")
    func dbLinearExtremeValues() {
        #expect(LoudnessEqualizerMath.dbToLinear(Float.nan) == 0.0)
        #expect(LoudnessEqualizerMath.dbToLinear(Float.infinity) == 0.0)
        #expect(LoudnessEqualizerMath.dbToLinear(-Float.infinity) == 0.0)
        #expect(LoudnessEqualizerMath.linearToDb(Float.nan) < -170.0)
        #expect(LoudnessEqualizerMath.linearToDb(Float.infinity) < -170.0)
        #expect(LoudnessEqualizerMath.linearToDb(-Float.infinity) < -170.0)
    }
